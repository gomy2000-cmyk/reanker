...
RESEND_API_KEY=...
REANKER_FROM_EMAIL=reanker@syojin.com  # Resend送信元（syojin.com認証・ローカル部でサービス分け）
CRON_SECRET=（任意の文字列）
```

> メール送信元は Resend の認証ドメインを `syojin.com` に集約。ReAnkerの通知は `reanker@syojin.com` から送信する（旧 `noreply@reanker.com` から移行）。`REANKER_FROM_EMAIL` 未設定時はコード側で `reanker@syojin.com` をデフォルト使用。
